...
export const PollingProvider = ({
  ...
  const FlipContractRef = useRef(FlipContract);
  const accountRef = useRef(account);

  // Update refs when dependencies change
  useEffect(() => {
    isNewUserRef.current = gameData.isNewUser;
    FlipContractRef.current = FlipContract;
    accountRef.current = account;
  }, [gameData.isNewUser, FlipContract, account]);
  ...
  const fetchData = useCallback(async () => {
    const currentContract = FlipContractRef.current;
